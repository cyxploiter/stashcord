...
router.get("/setup-status", async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      botInServer: user.hasStashcordGuild || false,
      guildId: user.stashcordGuildId,
      lastChecked: user.lastGuildCheck,
      setupComplete: !!(user.stashcordGuildId && user.hasStashcordGuild),
    });
  } catch (error) {
    ...
    if (guildsResponse.ok) {
      ...
      if (!user.stashcordGuildId) {
        const potentialGuilds = guilds.filter(
          (guild) =>
            ...
            (guild.owner || (parseInt(guild.permissions) & 0x8) === 0x8) // Administrator permission
        );

        console.log(
          `Found ${potentialGuilds.length} "Stashcord" guilds for user`
        );
        ...
        if (potentialGuilds.length > 0) {
          ...
          console.log(`Auto-selected Stashcord guild: ${updatedGuildId}`);
        }
      }
      ...
      if (updatedGuildId) {
        try {
          ...
          // Only consider it valid if the guild name is "Stashcord"
          if (botGuild && botGuild.name === "Stashcord") {
            ...
            console.log(
              `Bot is in valid Stashcord guild: ${botGuild.name} (${botGuild.id})`
            );
          } else if (botGuild) {
            console.log(
              `Bot is in guild "${botGuild.name}" but it's not named "Stashcord" - invalid`
            );
            hasStashcordGuild = false;
            // Reset the guild ID since it's not a valid Stashcord server
            updatedGuildId = null;
          } else {
...
router.post("/refresh-bot-status", async (req: Request, res: Response) => {
  try {
    ...
    // Check if user has a designated Stashcord guild
    if (!user.stashcordGuildId) {
      errorMessage =
        "No Stashcord guild found for user. Please complete setup first.";
      console.log(errorMessage);
    } else {
      try {
        // Check if bot is in the user's Stashcord guild
        const botGuild = await discordBot.getGuildById(user.stashcordGuildId);

        if (botGuild) {
          // Verify the guild name is "Stashcord"
          if (botGuild.name === "Stashcord") {
            botInGuild = true;
            ...
            errorMessage = `Guild "${botGuild.name}" is not named "Stashcord"`;
            console.log(errorMessage);
          }
        } else {
