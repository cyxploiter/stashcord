...
export default function FileCard({ file, onDownload, onDelete }: FileCardProps) {
  ...
  return (
    <TooltipProvider>
      <div 
        className="bg-white rounded-xl p-0 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer border border-gray-100 overflow-hidden"
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-7 w-7 p-0 rounded-full bg-white/90 hover:bg-red-500 hover:text-white shadow-sm"
                    onClick={(e) => {
                      e.stopPropagation();
                      onDelete(file.id);
                    }}
